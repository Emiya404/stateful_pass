...
__attribute__((destructor)) void end_fstate_tracer() {
  if(__fstate_shm_start!=(void*)-1 && __fstate_shm_start){
    int log_file_fd = open("/home/emiya/Desktop/experiment_aflnet/fstate_call_trace.log", O_RDWR | O_CREAT | O_APPEND, 0666);
    if(log_file_fd < 0){
      _exit(1);
    }
    ...
    *(u32*)__fstate_shm_ptr = 0xdeadbeef;
    ...
    while(count < MAP_SIZE && *(u32*)cur_obj != 0xdeadbeef){
      ...
      u32 func = *(((u32*)cur_obj) + 1);
      memset(log_buf, 0x0, sizeof(log_buf));
      sprintf(log_buf, "call %d on state %d\n", func, fstate);
      write(log_file_fd, log_buf, strlen(log_buf));

      cur_obj += 8;
      count += 8;
    }
    write(log_file_fd, "=========--end-flow-trace--=========\n", 38);
    close(log_file_fd);
    
  }
  __fstate_shm_ptr = __fstate_shm_start;
}
