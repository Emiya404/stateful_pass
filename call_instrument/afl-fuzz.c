...
static void remove_shm(void) {
  ...
  shmctl(fstate_shm_id, IPC_RMID, NULL);
...
EXP_ST void setup_shm(void) {
  ...
  fstate_shm_id = shmget(IPC_PRIVATE, (MAP_SIZE << 10), IPC_CREAT | IPC_EXCL | 0600);
  if(fstate_shm_id < 0) PFATAL("shmget() failed");
  shm_str = alloc_printf("%d", fstate_shm_id);
  setenv("__FSTATE_SHM_ID", shm_str, 1);

  fstate_bits = shmat(fstate_shm_id, NULL, 0);
  if(!fstate_bits) PFATAL("shmat() failed");
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  u8* fstate_map_fn = alloc_printf("%s/fstate_map_%d", out_dir, total_execs);
  int fstate_map_fd = open(fstate_map_fn, O_RDWR | O_CREAT, 0666);
  if(fstate_map_fd < 0){
    PFATAL("read fstate map failed");
  }
  u32* fstate_obj = (u32*)fstate_bits;
  int count = 0;
  while(count < (MAP_SIZE << 10) && *fstate_obj != 0xdeadbeef){
    write(fstate_map_fd, fstate_obj, 0x4);
    count += 4;
    fstate_obj++;
  }
  close(fstate_map_fd);
